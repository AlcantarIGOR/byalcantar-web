...
export default function Sidebar({ activeLog, onProjectClick }: SidebarProps) {
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>({
    edu: activeLog === "moodlesync",
    aps: ["onyx-launch", "onyx-pro", "onyx-automation", "onyx-care", "asistente-whatsapp", "onyx-digital-system", "onyx-launch-pro"].includes(activeLog || ""),
    design: true,
    contact: true
  });
  ...
  const apsProjects = [
    { label: "ONYX Launch", id: "onyx-launch", icon: <Code2 size={15} /> },
    { label: "ONYX Pro", id: "onyx-pro", icon: <Sparkles size={15} /> },
    { label: "ONYX Automation", id: "onyx-automation", icon: <Briefcase size={15} /> },
    { label: "ONYX Care", id: "onyx-care", icon: <Activity size={15} /> },
    { label: "Asistente WhatsApp", id: "asistente-whatsapp", icon: <MessageSquare size={15} /> }
  ];
    ...
    <aside className="w-full md:w-[260px] lg:w-[285px] md:h-screen md:sticky md:top-0 border-b md:border-b-0 md:border-r border-[#2c2c2c] bg-[#191919] flex flex-col justify-between shrink-0 p-6 z-20">
      <div className="space-y-8">
        ...
        <nav className="space-y-6">
          ...
          {/* Main Top Nav: About & Blog */}
          <div className="space-y-1">
            <Link
              href="/about"
              className={`flex items-center text-[14px] font-medium transition py-1 ${
            ...
            <Link
              href="/blog"
              className={`flex items-center text-[14px] font-medium transition py-1 group ${
                pathname?.startsWith("/blog") ? "text-[#a3e635] font-semibold" : "text-[#9b9b9b] hover:text-white"
              }`}
            >
              <BookOpen size={16} className="mr-2 text-[#9b9b9b] group-hover:text-white transition-colors" />
              <span>Blog & Notas</span>
            </Link>
          </div>
          ...
          <div className="space-y-2">
            <button
              ...
              className="w-full flex items-center justify-between text-[11.5px] font-mono uppercase tracking-[0.14em] text-[#a0a0a0] font-semibold hover:text-white transition py-1 group cursor-pointer"
            >
              <span>EDU</span>
              <ChevronRight
                size={14}
                className={`transition-transform duration-200 text-[#888888] group-hover:text-white ${
                  openSections.edu ? "rotate-90 text-white" : "rotate-0"
                }`}
              />
            </button>
            {openSections.edu && (
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <ChevronRight
                size={14}
                className={`transition-transform duration-200 text-[#888888] group-hover:text-white ${
                  openSections.aps ? "rotate-90 text-white" : "rotate-0"
                }`}
              />
            </button>
            {openSections.aps && (
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <ChevronRight
                size={14}
                className={`transition-transform duration-200 text-[#888888] group-hover:text-white ${
                  openSections.design ? "rotate-90 text-white" : "rotate-0"
                }`}
              />
            </button>
            {openSections.design && (
              <ul className="space-y-1">
                <li>
                  <Link
                    href="/meta-stamp"
                    className={`flex items-center justify-between text-[13.5px] font-medium transition py-1 group ${
                      pathname === "/meta-stamp" || activeLog === "meta-stamp" ? "text-white font-semibold" : "text-[#9b9b9b] hover:text-white"
                    }`}
                  >
                    <div className="flex items-center min-w-0">
                      <Sparkles size={15} className={`mr-2.5 shrink-0 transition ${pathname === "/meta-stamp" || activeLog === "meta-stamp" ? "text-[#a3e635]" : "text-[#9b9b9b]/70 group-hover:text-white"}`} />
                      <span className="truncate">Meta Stamp Studio</span>
                    </div>
                    {pathname === "/meta-stamp" || activeLog === "meta-stamp" ? (
                      <span className="w-1.5 h-1.5 rounded-full bg-[#a3e635] shrink-0 ml-2" />
                    ) : (
                      <span className="text-[10px] font-mono text-[#a3e635] bg-[#a3e635]/10 px-1.5 py-0.2 rounded border border-[#a3e635]/30 shrink-0 ml-2">NEW</span>
                    )}
                  </Link>
                </li>
                <li>
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <ChevronRight
                size={14}
                className={`transition-transform duration-200 text-[#888888] group-hover:text-white ${
                  openSections.contact ? "rotate-90 text-white" : "rotate-0"
                }`}
              />
            </button>
